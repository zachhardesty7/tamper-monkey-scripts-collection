...
function updateTickerConversionRate() {
  ...
  if (this.parentElement.parentElement.children.length > 2) {
    this.parentElement.parentElement.children[2].remove()
  }
  ...
  // convert to pretty USD format
  const USDVal = (this.value * tickerUSD).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,')
  const USDValElem = document.createElement('p')
...
HTMLElement.prototype.changeValueDetection = (function changeValueDetection() {
  const element = this
  ...
  if (window.changeValueDetectionEvents === undefined) {
    window.changeValueDetectionEvents = new Event('changeValueDetection')
  }

  element.setAttribute('data-uiq', uiq)

  window[uiq] = setInterval(() => {
